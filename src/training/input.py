import numpy as np
import torch
from src.environment.action import Action, Direction
from src.environment.board import Board
from src.environment.game_master import GameMaster
from src.environment.tile import Tile, TileType


def convert_state_to_array(state: GameMaster, n_players: int, fog_of_war: bool):
    state_array = np.stack(
        [
            convert_state_to_array_for_player(state, player_index, fog_of_war)
            for player_index in range(n_players)
        ]
    )
    return state_array


def convert_state_to_array_for_player(
    state: GameMaster, player_index: int, fog_of_war: bool
):
    state_array = np.stack(
        [
            convert_tile_to_array(state, tile, player_index, fog_of_war)
            for row in state.board.grid
            for tile in row
        ]
    ).T.reshape(-1, state.board.num_rows, state.board.num_cols)
    return state_array


def convert_tile_to_array(
    state: GameMaster, tile: Tile, player_index: int, fog_of_war: bool
):
    tile_state = np.zeros(get_input_channel_dimension_size(fog_of_war), dtype=np.int32)
    has_vision = tile.player_visibilities[player_index] or not fog_of_war

    # indices 0, 1 represents my tile, opponent's tile
    if tile.player_index is not None:
        tile_state[int(tile.player_index != player_index)] = 1

    # index 2 represents the number of troops on the tile
    tile_state[2] = tile.army if tile.army is not None else 0

    # index 3 represents the turn number
    tile_state[3] = state.turn

    # index 4+ represents the tile type
    if tile.type == TileType.NORMAL and has_vision:
        tile_type = 0
    elif tile.type == TileType.MOUNTAIN and has_vision:
        tile_type = 1
    elif tile.type == TileType.GENERAL and has_vision:
        tile_type = 2
    elif tile.type == TileType.CITY and has_vision:
        tile_type = 3
    elif (
        tile.type == TileType.CITY or tile.type == TileType.MOUNTAIN
    ) and not has_vision:
        tile_type = 4
    elif (
        tile.type == TileType.GENERAL or tile.type == TileType.NORMAL
    ) and not has_vision:
        tile_type = 5

    tile_state[tile_type + 4] = 1
    return tile_state


def get_input_channel_dimension_size(fog_of_war: bool):
    return 10 if fog_of_war else 8
