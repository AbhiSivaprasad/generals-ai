...
from base.client.map import Player, Tile, MapBase, PLAYER_CHAR_BY_INDEX, new_value_grid, MODIFIER_TORUS
...
class EklipZBot(object):
    ...
    def init_turn(self, secondAttempt=False):
        ...
        self._alt_en_gen_position_distances = [None for _ in self._map.players]
        ...
        for player in self._map.players:
            if player.team != self.player.team:
                altEnGenPositions = self.get_target_player_possible_general_location_tiles_sorted(elimNearbyRange=2, player=player.index, cutoffEmergenceRatio=0.3)
    ...
    def get_timings(self) -> Timings:
        ...
        if self.target_player_gather_targets is not None:
            ...
            launchTiming = 50 - self.shortest_path_to_target_player.length - 5
        ...
        # should usually be 0 except the first turn
        correction = self._map.turn % 50
        timings = Timings(cycleDuration, quickExpandSplit, gatherSplit, launchTiming, 0, self._map.turn + cycleDuration - correction, disallowEnemyGather)
        timings.is_early_flank_launch = isOurPathAMostlyFogAltPath
    ...
    def pick_move_after_prep(self, is_lag_move=False):
        ...
        if self.get_remaining_move_time() > 0.01:
            with self.perf_timer.begin_move_event('FOUND NO MOVES GATH'):
                ...
                gathMove = self.timing_gather(targets, negatives, None, force=True, pruneToValuePerTurn=True, useTrueValueGathered=True, includeGatherTreeNodesThatGatherNegative=False, targetTurns=turns, logStuff=True)
                if gathMove:
                    return gathMove
        ...
        if self.gather_use_pcst and gatherTurns > 0 and targetArmy < 0 and not isinstance(targets, dict):
            ...
            if self.gather_use_max_set and not isinstance(targets, dict):
                ...
                armyCostMatrix = Gather.build_gather_capture_pure_value_matrix(
                    ...
                    useTrueValueGathered=True,
                    prioritizeCaptureHighArmyTiles=False)
                with self.perf_timer.begin_move_event('gath max set in gath to target tiles'):
    ...
    def get_optimal_exploration(
        ...
        with self.perf_timer.begin_move_event(f'Watch {startTile} c{str(includeCities)[0]} tgA {targetArmyLevel}'):
            if maxTime is None:
                maxTime = self.get_remaining_move_time()
    ...
    def find_enemy_city_path(self, negativeTiles: TileSet, force: bool = False) -> typing.Tuple[int, Path | None]:
        ...
        foundScores = [c for c in scores if c.discovered]
        ...
        for tile in scores:
            ...
            if preCalcedOffensePlan is None:
                logbook.info(f'Skipping {tile} because win condition analyzer didnt think it was contestable.')
                continue
            ...
            if self.territories.is_tile_in_friendly_territory(tile) or self._map.is_player_on_team_with(self.targetPlayer, tile.player):
                ...
                with self.perf_timer.begin_move_event(f'city @{tile} approx attack/def {timing}t'):
                ...
                self.viewInfo.add_stats_line(f'city @{tile} in {curLeft} diff {diff} (us={ourAttack} vs them={theirDef})')
    ...
    def get_general_move_blocking_tiles(self, target: Tile, move_half=False):
        ...
        for dangerTile in dangerTiles:
            ...
            targetBlocksDangerTile = (
                    ...
                    or (len(dangerTileForwardMoves) == 1 and genDist > self._map.euclidDist(dangerTile.x, dangerTile.y, target.x, target.y))
            )
    ...
    def get_path_to_targets(
        ...
        def path_to_targets_priority_func(
            ...
            if skipEnemyCities and nextTile.isCity and self._map.is_tile_enemy(nextTile):
                return None
    ...
    def get_predicted_target_player_general_location(self, skipDiscoveredAsNeutralFilter: bool = False) -> Tile:
        ...
        if fallbackTile is None or fallbackTile == self.general:
            self.viewInfo.add_info_line("target path fallback failed, furthest pathable reachable tile")
            furthestDist = 0
            furthestTile = None
            for tile in self._map.pathable_tiles:
                if tile.visible:
    ...
    def try_threat_gather(
        ...
        for threat in threats:
            ...
            if self.has_defenseless_modifier and self.general in negatives:
    ...
    def get_gather_tiebreak_matrix(self) -> MapMatrixInterface[float]:
        ...
        for tile in self._map.get_all_tiles():
            isFr = self._map.is_tile_friendly(tile)
            if not isFr:
                continue
            ...
            if tile in self.board_analysis.intergeneral_analysis.shortestPathWay.tiles:
                matrix.raw[tile.tile_index] -= 0.6

            if tile.army <= 1:
                matrix.raw[tile.tile_index] -= 1.0
            ...
            if self.info_render_gather_values and isFr:
    ...
    def get_defense_moves(
        ...
        for threat in threats:
            ...
            if threat is not None and threat.threatType == ThreatType.Kill:
                ...
                if move:
                    with self.perf_timer.begin_move_event(f'Def prun @ {str(threat.path.start.tile)}->{str(threat.path.tail.tile)}'):
                        if valueGathered > survivalThreshold:
                            ...
                            # TODO distOffset was causing early gathers...? Why was it here?
                            leavesGreaterThanDistance = GatherTreeNode.get_tree_leaves_further_than_distance(pruned, threat.armyAnalysis.aMap, threat.turns, survivalThreshold, sumPruned)
                            anyLeafIsSameDistAsThreat = len(leavesGreaterThanDistance) > 0
                            if anyLeafIsSameDistAsThreat:
                                flags = f'leafDist {flags}'
                            else:
                                leavesGreaterThanBlockDistance = GatherTreeNode.get_tree_leaves_further_than_distance(pruned, threat.armyAnalysis.aMap, saveTurns + 1)
                            ...
                            if sumPrunedTurns >= threat.turns - 1 or anyLeafIsSameDistAsThreat:  # was - 2, but that fails this test: test_should_not_defense_loop_let_army_engine_make_moves
                                ...
                                self.info(
                                    f'{flags}GathDefRaw-{str(threat.path.start.tile)}@{str(threat.path.tail.tile)}:  {str(move)} val {valueGathered:.1f}/p{sumPruned:.1f}/{survivalThreshold} turns {turnsUsed}/p{sumPrunedTurns}/{threat.turns} offs{distOffsetNOWUNUSED}')
                ...
                self.info(
                    f'{flags}GathDef-{str(threat.path.start.tile)}@{str(threat.path.tail.tile)}:  {str(move)} val {valueGathered:.1f}/{survivalThreshold} turns {turnsUsed}/{threat.turns} (abandThresh {abandonDefenseThreshold:.0f} offs{distOffsetNOWUNUSED}')
    ...
    def initialize_from_map_for_first_time(self, map: MapBase):
        ...
        self.has_defenseless_modifier = 'Defenseless' in self._map.modifiers
        self.has_watchtower_modifier = 'Watchtower' in self._map.modifiers
        self.has_misty_veil_modifier = 'Misty Veil' in self._map.modifiers
        ...
        for enemyCity in enemyCitiesOrderedByPriority:
            ...
            for movable in enemyCity.movableNoObstacles:
                if movable.player == enemyCity.player and movable.lastMovedTurn > self._map.turn - 2 and movable.delta.toTile != enemyCity:
                    enMovedNear = True
                    break
                if movable.player == self.general.player and movable.army > enemyCity.army + 1 and movable not in defenseCriticalTileSet:
                    cap = Path()
                    cap.add_next(movable)
                    cap.add_next(enemyCity)
            if not enMovedNear and cap:
                self.info(f'returning 1-move-city-kill {cap}')
                return cap
    ...
    def should_allow_neutral_city_capture(
        ...
        if self.player.standingArmy - 15 < cityCost:
            return False
    ...
    def get_main_gather_move(
        ...
        if (enemyGather or neutralGather) and not self.is_all_in() and self._map.turn >= 150:
            ...
            if not self.is_all_in() and not self.defend_economy and enemyGather and self._map.turn >= 150 and leafMoves:
            ...
            if self.timings.in_quick_expand_split(self._map.turn) and forceGatherToEnemy:
                ...
                if move is not None:
                    if not isinstance(self.curPath, MoveListPath):
                        self.curPath = None
                    self.info(
                        f"GATHER QE needToKill{gathString}! Gather move: {str(move)} Duration {time.perf_counter() - gathStartTime:.4f}")
        ...
        if move is not None:
            ...
            if not isinstance(self.curPath, MoveListPath):
                self.curPath = None
            self.info(
                f"GATHER {gathString}! Gather move: {str(move)} Duration {time.perf_counter() - gathStartTime:.4f}")
            return self.move_half_on_repetition(move, 6, 4)
        else:
...
Unknown message type: ['ping_tile', 125, 0]        @param pingTile:
    ...
    def try_find_expansion_move(self, defenseCriticalTileSet: typing.Set[Tile], timeLimit: float, forceBypassLaunch: bool = False, overrideTurns: int = -1) -> Move | None:
        ...
        if path:
            ...
            if self.is_move_safe_valid(move):
                self.info(
                    f"EXP {str(move)} {path.econValue:.2f} neg ({expansionNegStr})")
                return move
        elif len(allPaths) > 0:
            self.info(
    ...
    def get_expansion_weight_matrix(self, copy: bool = False, mult: int = 1) -> MapMatrixInterface[float]:
        ...
        if mult != 1:
            copy = self._expansion_value_matrix.copy()
            for t in self._map.get_all_tiles():
                copy.raw[t.tile_index] *= mult
    ...
    def _get_standard_expansion_capture_weight_matrix(self) -> MapMatrixInterface[float]:
        ...
        self.info(f'endOfCyclePenaltyRatio {endOfCyclePenaltyRatio:.3f}')
        ...
        if self.enemy_expansion_plan is not None:
            for enPath in self.enemy_expansion_plan.all_paths:
                for tile in enPath.tileList[1:]:
                    if self._map.is_tile_friendly(tile):
                        matrix[tile] -= self.expansion_enemy_expansion_plan_inbound_penalty * endOfCyclePenaltyRatio
        ...
        for tile in self._map.get_all_tiles():
            ...
            enDistRatio = (genDist + 3) / max(1, (enDist + numEnGenPos // 5))
            ...
            if not tile.discovered and self.armyTracker.valid_general_positions_by_player[self.targetPlayer][tile] and numEnGenPos < 10:
            ...
            cappedRat = max(1.0, enDistRatio)
            if tile.player == -1:
                bonus += 0.1 - 0.05 * cappedRat
            else:
                bonus += 0.1 - 0.03 * cappedRat

            # here
            excessDist = enPotentialGenDistances[tile] - self.board_analysis.inter_general_distance - numEnGenPos
            if excessDist > 0:
                    ...
                    for planOpt in self.intercept_plans.values():
                        ...
                        if interceptOption is not None:
                            ...
                            if isOneMoveLargeIntercept or interceptOption.econValue / interceptOption.length > interceptVtCutoff:
                                self.viewInfo.add_info_line(f'EXP USED INT {interceptOption}')
    ...
    def make_second_25_move(self) -> Move | None:
        ...
        #         continue
        #
        #     targetIsland: TileIsland = self.tileIslandBuilder.tile_island_lookup.raw[dest.tile_index]
        #
        #     if targetIsland.team != -1:
        #         if targetIsland.tile_count_all_adjacent_friendly < 5:
        #             continue
        #     else:
        #         if targetIsland.tile_count_all_adjacent_friendly < 15:
        #             continue
        #
            ...
            if winningByEnoughToBeSuperCareful and flankIsCloserThanThreeFifths:
                ...
                with self.perf_timer.begin_move_event(f'superCareful flank gath {turns}t'):
                    move, valGathered, turnsUsed, nodes = self.get_gather_to_target_tiles(
                        checkPath.convert_to_dist_dict(offset=0 - checkPath.length),
                        maxTime=0.002,
                        gatherTurns=turns,
                        negativeSet=defenseCriticalTileSet,
                        targetArmy=1,
    ...
    def try_get_cyclic_all_in_move(self, defenseCriticalTileSet: typing.Set[Tile]) -> Move | None:
        ...
        if not self.targetPlayerExpectedGeneralLocation.isGeneral:
            explorePath = self.explore_target_player_undiscovered(defenseCriticalTileSet, maxTime=0.05)
            ...
            if remainingTurns > 0:
                ...
                with self.perf_timer.begin_move_event(f'{reason}gather cyclic {remainingTurns}'):
                    ...
                    if gcp is not None:
                        ...
                        self.info(f'pcst {reason}gath cyc {remainingTurns} {str(move)} @ {self.str_tiles(targets)} neg {self.str_tiles(negatives)}')
                        return move
    ...
    def get_all_in_move(self, defenseCriticalTileSet: typing.Set[Tile]) -> Move | None:
        if self.is_all_in():
            ...
            with self.perf_timer.begin_move_event(f'pcst {msg}. self.all_in_army_advantage_cycle {self.all_in_army_advantage_cycle}, self.all_in_army_advantage_counter {self.all_in_army_advantage_counter}'):
                ...
                self.info(f'PCST ALL IN {gathCapPlan}')
    ...
    def get_kill_race_chance(self, generalHuntPath: Path, enGenProbabilityCutoff: float = 0.4, turnsToDeath: int | None = None, cutoffKillArmy: int = 0, againstPlayer: int = None) -> float:
        ...
        if isOnlyOneSpot:
            ...
            logbook.info(f'We lose the race.')
            return 0.0
        ...
        for tile in toReveal:
            ...
            if killDist < turnsToDeath:
                ...
                self.info(f'{tile} too far {killDist}/{turnsToDeath} to kill, wed lose! :(')
        ...
        self.info(f'KillRaceProb {killInTurnsChance:.2f} ({killChance:.2f}) - {bestRevealedPath.length}t ({generalHuntPath.length}t) reached {reachedCount} rev {revealedCount}/{len(toReveal)}, kill min{minKillTurns} max{maxKillTurns} avg{avgKillTurns:.1f}')
